...
def train(model, train_dataloader, val_dataloader, criterion, optimizer, device, epochs, patience, fold):
    ...
    for epoch in range(epochs):
        ...
        results_path = f'...\\train_result\\training_results_{fold}.csv'
...
def main():
    ...
    short_term_tcxt_num_embeddings = [12, 2, 2]  # The number of categories in each short-term temporal context feature
    short_term_tcxt_embedding_dims = [4, 2, 2]  # The embedding dimension of each short-term temporal context feature

    long_term_tcxt_num_embeddings = [2, 2]  # The number of categories in each long-term temporal context feature
    long_term_tcxt_embedding_dims = [2, 2]  # The embedding dimension of each long-term temporal context feature

    metadata_csv_file = r'...\metadata.csv'
    content_features_path = r'...\content_features.npy'
    temporal_features_path = r'...\temporal_data_target.npy'

    dataset = BilibiliDataset(metadata_csv_file, content_features_path, temporal_features_path)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print('Using device:', device)

    metrics = ['Fold', 'Test Loss', 'Test MAE', 'Test Spearman', 'Test R-squared']
    results_path = r'...\train_result\test_result_TIML.csv'
    ...
    for fold in range(1, 6):
        ...
        # 为当前fold设置固定随机种子
        fold_seed = fold_seeds[fold]
        set_random_seed(fold_seed)
        split_generator = torch.Generator().manual_seed(fold_seed)
        ...
        model.load_state_dict(torch.load(f'E...\\model_{fold}.pth'))
        model.eval()
        test_outputs = []
        test_targets = []
